...
{
    public abstract class AuthServiceBase<TRequest> : ServiceBase<TRequest>
    {
		...
		private void Validate(TRequest request)
		{
			...
			if (attrs.Length > 0)
			{
				...
				if( !customAuth.Action.IsNullOrEmpty() ) {
					var du = Session.GetUserData<DatosUsuario>();		
				    if ( du.Actividades.Count(r=> r.Nombre== customAuth.Action)==0 ){
						throw new UnauthorizedException("Usuario no autorizado para ejecutar: " + customAuth.Action);
					}
				}	
			}
        }
